...
export default function AdminReservasiPage(): React.JSX.Element {
  ...
  const fetchReservations = async () => {
    ...
    try {
      ...
      if (!response.ok) throw new Error('Gagal memuat data reservasi dari server.');
      const data = await response.json();
      setReservasis(data);
    } catch (error: any) {
      console.error('Error Fetch:', error);
      toast.error(error.message || 'Gagal menyinkronkan daftar reservasi.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    // 1. Validasi Nomor Meja
    const parsedTableNumber = parseInt(form.meja, 10);
    if (isNaN(parsedTableNumber)) {
      toast.error('Nomor meja harus berupa angka murni! (Contoh: 6)');
      setIsSubmitting(false);
      return;
    }

    // 2. Format Tanggal & Jam Menjadi ISO Standard
    let validIsoDate: string;
    try {
      const localDateTime = new Date(`${form.tanggal}T${form.jam}`);
      if (isNaN(localDateTime.getTime())) {
        throw new Error("Format input waktu cacat.");
      }
      validIsoDate = localDateTime.toISOString();
    } catch (err) {
      toast.error('Format penanggalan salah. Mohon periksa kembali inputan Anda.');
      setIsSubmitting(false);
      return;
    }
    ...
    // Tampilkan Toast Loading State
    const actionToastId = toast.loading(form.id ? 'Memperbarui rincian reservasi...' : 'Mendaftarkan reservasi baru...');
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorMsg || 'Gagal menyimpan perubahan ke database.');
      }

      // Ubah status toast loading menjadi sukses secara instan
      toast.success(form.id ? 'Data reservasi berhasil diperbarui!' : 'Reservasi baru berhasil disimpan!', { id: actionToastId });

      // Reset form state ke kondisi semula
      setForm({ id: null, nama: '', telepon: '', meja: '', tanggal: '', jam: '', jumlahOrang: 2, status: 'PENDING' });
      fetchReservations(); 
    } catch (error: any) {
      console.error('Submit Error:', error);
      toast.error(error.message || 'Terjadi gangguan internal pada server.', { id: actionToastId });
    } finally {
      setIsSubmitting(false);
    }
  };

  // --- FEATURE 3: DELETE RESERVASI ---
  const handleDelete = async (id: number) => {
    if (!confirm('Hapus data reservasi ini secara permanen?')) return;

    const deleteToastId = toast.loading('Menghapus data dari sistem...');
    try {
      const token = localStorage.getItem('token');
      ...
      if (!response.ok) {
        const result = await response.json();
        const errorMsg = Array.isArray(result.message) ? result.message[0] : result.message;
        throw new Error(errorMsg || 'Akses ditolak oleh server.');
      }

      toast.success('Data reservasi berhasil dihapus permanen.', { id: deleteToastId });
      fetchReservations();
    } catch (error: any) {
      console.error('Delete Error:', error);
      toast.error(error.message || 'Gagal menghapus reservasi.', { id: deleteToastId });
    }
  };
  ...
  return (
    <main className="bg-[#090705] min-h-screen text-[#f3f1ed] antialiased">
      {/* Konfigurasi global desain Luxury Custom Toast Notification */}
      <Toaster 
        position="top-right"
        toastOptions={{
          style: {
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
          ...
          <div>
            ...
            <div className="flex gap-2">
              ...
              />
              {filterDate && (
                <button onClick={() => setFilterDate('')} className="bg-white/5 border border-white/10 text-xs px-3 hover:bg-white/10 text-gray-400 hover:text-white transition">
                  Reset
                </button>
              )}
            </div>
          ...
          <div className="lg:col-span-4 bg-[#120f0b] p-6 border border-white/[0.03] sticky top-28 shadow-xl">
            ...
            <form onSubmit={handleSave} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-2">
                <input type="date" value={form.tanggal} onChange={e => setForm({ ...form, tanggal: e.target.value })} required className="bg-black/40 border border-white/5 p-3 text-xs outline-none focus:border-brand-gold text-white transition" />
                <input type="time" value={form.jam} onChange={e => setForm({ ...form, jam: e.target.value })} required className="bg-black/40 border border-white/5 p-3 text-xs outline-none focus:border-brand-gold text-white transition" />
              </div>
              ...
              {form.id && (
                <button type="button" onClick={() => setForm({ id: null, nama: '', telepon: '', meja: '', tanggal: '', jam: '', jumlahOrang: 2, status: 'PENDING' })} className="w-full bg-white/5 border border-white/10 text-xs py-2 text-gray-400 hover:text-white uppercase tracking-widest transition mt-1">
                  Batal Edit
                </button>
              )}
            </form>
                          ...
                          <td className="py-3.5 px-2 text-center">
                            <div className="flex items-center justify-center gap-3">
                              ...
                              <button
                                ...
                                className="text-gray-600 hover:text-red-400 text-xs transition font-medium cursor-pointer"
                              >
                                ✕ Hapus
                              </button>
                            </div>
